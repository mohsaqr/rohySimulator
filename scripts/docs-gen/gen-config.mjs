...
/**
 * Names rohy READS but does not CONFIGURE.
 *
 * `server/lib/pluginSpawn.js` builds an allowlisted environment for a
 * subprocess — `PATH`, `HOME`, `LANG` — precisely so the child does NOT inherit
 * JWT_SECRET and every provider key. Those reads are real, but the variables
 * are the operating system's, not knobs an operator sets to configure rohy, and
 * listing them in a deployment reference makes the list that matters harder to
 * read.
 *
 * Anything ROHY_-prefixed is configuration by definition and is never skipped.
 */
const INHERITED_ENV = new Set(['PATH', 'HOME', 'LANG', 'LC_ALL', 'TMPDIR', 'SHELL', 'USER', 'PWD']);
...
const PURPOSES = {
    ...
    ROHY_PLUGIN_IMPORT_ORIGINS: 'Comma-separated `<pluginId>=<origin>` allowlist naming the hosts a plugin may DOWNLOAD from (RPS-1 1.4). The operator\u0027s outer bound: a tenant admin narrows it through the plugin\u0027s own settings and can never widen it, because a tenant admin is not the server operator and naming a host for rohy\u0027s server to fetch from is the SSRF shape proxy-routes.js already closed once. A plugin id may repeat and the origins accumulate. Unset means NO plugin may import from anywhere \u2014 the correct default for a server nobody has told where content may come from. Malformed is fatal at boot.',
    ROHY_KB_DB: 'Where the derived knowledge base is written (default `data/knowledge.db`). Tooling only — the server never reads it. Set it to build a scratch copy without clobbering the real one, which is what the parser tests do. The file itself is gitignored and rebuilt by `npm run kb:build`; see docs/integrator/knowledge-base.md.',
    ROHY_PLUGIN_LIBRARY_DIRS: 'Comma-separated `<pluginId>=<absolute path>` map of the managed library directory each plugin\u0027s server module may write in (RPS-1 1.4). Plural, one per plugin, because a singular variable cannot serve a second plugin and "the second plugin needs no host edit" is the claim RPS-1 makes. Paths must be absolute: a relative path resolves against whatever working directory the service happens to have, which differs between a dev shell, a systemd unit and a Docker image. Unset means the plugin has no library and its import surface is unavailable \u2014 not an error. Malformed is fatal at boot.',
    ...
    ROHY_PACS_CONTENT: 'Path to a built imaging content origin, overriding the sibling `../Radoyon/radoyon/dist-content` that `npm run starter-content` reads by default. Build-time only \u2014 nothing at runtime reads it.',
    ROHY_PATHOLOGY_CONTENT: 'Path to a built slide content origin, overriding the sibling `../Pathoyon/dist-content` that `npm run starter-content` reads by default. Build-time only \u2014 nothing at runtime reads it.',
    ROHY_CONTENT_TOKEN: 'A GitHub token with read access to the content repository, used by `npm run setup:content` to download the published imaging and slide archives. Only needed while that repository is private, and only at install: nothing at runtime reads it. `GITHUB_TOKEN` is accepted as a fallback, as is the `gh` CLI when the machine is logged in. Installing from a local file with `--from` needs no token at all, because the installer trusts the archive\u0027s SHA-256 rather than the host it came from.',
    GITHUB_TOKEN: 'Fallback credential for `npm run setup:content` when ROHY_CONTENT_TOKEN is unset. Install-time only; no runtime code reads it.',
...
async function main() {
    ...
    const secrets = [];
    // Inherited OS variables are read but not configured — see INHERITED_ENV.
    const names = [...results.keys()].filter((n) => n.startsWith('ROHY_') || !INHERITED_ENV.has(n)).sort();
