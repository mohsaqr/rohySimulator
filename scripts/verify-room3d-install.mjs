#!/usr/bin/env node
/**
 * Refuse to build without the 3D patient room.
 *
...
if (!existsSync(resolve(sibling, 'src', 'main.js'))) {
    problems.push(`${sibling}/src/main.js is missing — clone https://github.com/mohsaqr/3D beside this checkout (see docs/INSTALL.md).`);
}
...
if (problems.length) {
    console.error('verify:room3d FAILED — the 3D patient room cannot be bundled:');
    problems.forEach((p) => console.error(`  - ${p}`));
    process.exit(1);
}
const version = JSON.parse(readFileSync(resolve(sibling, 'package.json'), 'utf8')).version;
console.log(`verify:room3d OK — rohy-3d-patient-room ${version} at ${sibling}`);
