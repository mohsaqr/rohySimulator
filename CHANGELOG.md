...
- **Screenshots page** (`website/screenshots.html`, "Screenshots" in every page's nav): 15 sections, 36 real screenshots of this release with captions, generated by `website/build-screenshots.mjs` from `website/assets/screenshots/shots.json`. `website/screenshot-shots.mjs` (`npm run website:shots`) photographs 96 scenes of a running dev server as the seeded admin, writes WebP plus a 480 px miniature per scene, hides the Diag pill and picks the richest Oyon session. `npm run website:build` now also builds the page.

### Changed

- The front page and the rooms page show the current screens inside a browser-window frame; the room cards and the rooms page's "On this page" row carry tinted miniatures. The 12-lead ECG room is photographed for the first time.
- README, help page and link-preview pictures regenerated from the new screenshots under their old names; README captions follow.
...
- The floating Diag pill (admin diagnostic-bar toggle) sat over the room navigator's Course tab; it now rests above the navigator. Regression lock in `DiagnosticBar.test.jsx`.
...
- The starter-content tests depended on content being installed on the
  machine. The bundles are gigabytes, generated by `npm run setup:content`
  and gitignored, so a CI runner has none — the tests passed locally and
  failed in CI, which is the wrong way round for a test to be wrong. They
  now build a small fixture bundle and point `ROHY_STARTER_CONTENT_DIR` at
  it, so they test the serving contract rather than whatever happens to be
  installed.

### Added

- **`ROHY_STARTER_CONTENT_DIR`** — where the installed content lives,
  overriding the in-tree `server/plugin-content/`. The bundles are
  generated rather than committed, so a container should serve them from a
  mounted volume rather than an image layer; this is the mount point. It
  also answers the open question in the delivery plan about whether the
  Docker image should bake content in — it need not.
