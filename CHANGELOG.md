...
- **`npm ci` failed on the server with "Missing … from lock file".** The
  lockfile had been regenerated by npm 11 (local), which prunes
  `@docsearch/js`'s nested React 18 tree; the server runs **npm 10.9.8**, which
  requires those entries to be present. Regenerated with npm 10 so both accept
  it — verified with `npm ci --dry-run` under 10 and 11. This is the recurring
  npm-10-vs-11 lockfile gotcha: **generate the lockfile with the npm major the
  server runs**, not the one on your Mac.
