/*
 * VENDORED from the Cardoyon ECG package (Github/ECG/src/patientFigure.js),
 * byte-for-byte apart from this header. Rohy also carries this module at
 * src/components/ecg/patientFigure.js, inside the ecg PLUGIN's vendored
 * package — and that is exactly why a second copy lives here: the
 * examination room is core, and core must not import from a plugin's
 * directory, or deleting that plugin (the peaceful-exclusion rule) would
 * take the examination room down with it. The two copies are meant to stay
 * byte-identical below this header; refresh both from upstream together.
 *
 * The 232x282 frame is load-bearing: AuscultationPanel's manikin points are
 * measured against this crop's anatomy, exactly as leadTopography.js measures
 * the ECG electrodes against it.
 */
/**
