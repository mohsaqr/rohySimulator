...
export default function OyonWindowsView({ records, loading }) {
   ...
   if (loading && !rows.length) {
      return <Empty msg={t('windows_loading')} />;
   }
   ...
   return (
      <div className="rohy-admin-light space-y-2">
         <div className="rounded-lg border border-gray-200 bg-white p-3 grid gap-3 md:grid-cols-3">
            <FormRow label={t('filter_min_confidence')} hint={`${(minConfidence * 100).toFixed(0)}%`}>
               <input
                  type="range"
                  min={0} max={1} step={0.05}
                  value={minConfidence}
                  aria-label={t('filter_min_confidence')}
                  onChange={e => setMinConfidence(Number(e.target.value))}
                  className="w-full"
               />
            </FormRow>
            <FormRow label={t('filter_max_missing_face')} hint={`${(maxMissingFace * 100).toFixed(0)}%`}>
               <input
                  type="range"
                  min={0} max={1} step={0.05}
                  value={maxMissingFace}
                  aria-label={t('filter_max_missing_face')}
            ...
            <div className="block">
               <span className="block text-xs font-semibold text-gray-600 mb-1">
                  {t('filter_dominant')}<span className="text-gray-500 font-normal"> · {t('filter_multi_select')}</span>
               </span>
         ...
         <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs text-gray-600">
               {t('windows_filtered_count', { filtered: filtered.length, total: rows.length })}
            </span>
            ...
            <div className="max-h-[70vh] overflow-auto rounded-lg border border-gray-200">
               <table className="min-w-[1280px] w-full text-xs">
                  <thead className="sticky top-0 z-10 bg-white shadow-lg text-gray-600 uppercase">
                     <tr>
                        <th className="text-left px-2 py-2">{t('col_time')}</th>
                        <th className="text-left px-2 py-2">{t('col_user')}</th>
                        <th className="text-left px-2 py-2">{t('col_role')}</th>
                        <th className="text-left px-2 py-2">{t('col_case')}</th>
                        <th className="text-right px-2 py-2">{t('col_session')}</th>
                        <th className="text-left px-2 py-2">{t('col_dominant_estimate')}</th>
                        <th className="text-right px-2 py-2">{t('col_conf')}</th>
                        <th className="text-right px-2 py-2">{t('col_valence')}</th>
                        <th className="text-right px-2 py-2">{t('col_arousal')}</th>
                        <th className="text-right px-2 py-2">{t('col_entropy')}</th>
                        <th className="text-right px-2 py-2">{t('col_frames')}</th>
                        <th className="text-right px-2 py-2">{t('col_miss')}</th>
                        <th className="text-left px-2 py-2">{t('col_model')}</th>
                        <th className="text-left px-2 py-2">{t('col_consent')}</th>
                        ...
                        return (
                           <React.Fragment key={r.id}>
                              <tr className="border-t border-gray-200 hover:bg-gray-50">
                                 ...
                                 <td className="px-2 py-1.5">{r.username || r.student_name_snapshot || (r.user_id != null ? `#${r.user_id}` : t('anonymised'))}</td>
                                 <td className="px-2 py-1.5 text-gray-600">{r.user_role || r.student_role_snapshot || '—'}</td>
                                 <td className="px-2 py-1.5">{r.case_title_snapshot || (r.case_id ? t('case_fallback', { id: r.case_id }) : '—')}</td>
                                 ...
                                 <td className="px-2 py-1.5">
                                    ...
                                    >
                                       {isOpen ? t('detail_hide') : t('detail_show')}
                                    </button>
                                 </td>
                              </tr>
                              {isOpen && (
...
function WindowDetail({ record, t }) {
   ...
   return (
      <div className="grid gap-4 md:grid-cols-2">
         <div>
            <div className="text-xs uppercase tracking-wide text-gray-600 mb-2">{t('window_probability_map')}</div>
            {probEntries.length === 0 && <div className="text-sm text-gray-500">{t('window_no_probabilities')}</div>}
         ...
         <div>
            <div className="text-xs uppercase tracking-wide text-gray-600 mb-2">{t('window_quality_raw')}</div>
