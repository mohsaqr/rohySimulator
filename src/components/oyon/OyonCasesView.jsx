...
export default function OyonCasesView({ records }) {
   ...
   if (!cases.length) return <Empty msg={t('cases_empty')} />;
   return (
      <div className="rohy-admin-light max-h-[70vh] overflow-auto rounded-lg border border-gray-200">
         <table className="min-w-[920px] w-full text-sm">
            <thead className="sticky top-0 z-10 bg-white shadow-lg text-gray-600 text-xs uppercase">
               <tr>
                  <th className="text-left px-3 py-2">{t('col_case')}</th>
                  <th className="text-left px-3 py-2">{t('col_category')}</th>
                  <th className="text-right px-3 py-2">{t('col_students')}</th>
                  <th className="text-right px-3 py-2">{t('col_sessions')}</th>
                  <th className="text-right px-3 py-2">{t('col_windows')}</th>
                  <th className="text-left px-3 py-2">{t('col_distribution')}</th>
                  ...
                  <tr key={`${c.case_id ?? 'null'}-${i}`} className="border-t border-gray-200 hover:bg-gray-50">
                     <td className="px-3 py-1.5">
                        <span className="font-semibold text-gray-900">
                           {c.case_title
                              || (c.case_id ? t('case_fallback', { id: c.case_id }) : t('unknown_case'))}
                        </span>
                     </td>
                     ...
                     <td className="px-3 py-1.5"><DistBar dist={c.dominant_estimate_distribution} t={t} /></td>
      ...
      <div className="flex h-2.5 w-40 rounded overflow-hidden bg-gray-100">
         {entries.map(([label, v]) => (
            <div
               ...
               title={t('distribution_tooltip', { label, count: v })}
            />
         ))}
      </div>
   );
