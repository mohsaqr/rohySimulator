...
These signals can be visualised as timelines, distributions, transitions and heatmaps. Emotion heatmaps can show where affective signals concentrate across time, session phases, rooms or cases. Gaze summaries can be related to monitor use, report viewing or avatar interaction. The per-screen gaze maps draw the 3×3 zone grid over a toned-down miniature of that room, so a zone reads as the part of the screen it covers: the monitor, the transcript, the report. Valence and arousal timelines can be compared with alarms, deterioration, treatment or debrief moments. These views support hypotheses for reflection and research; they are not direct readings of a learner's inner state.
