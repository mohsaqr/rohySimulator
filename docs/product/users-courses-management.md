...
Roles are rank-based: each role carries a numeric rank and every check reads "rank ≥ N". A student can run cases. A reviewer can inspect more information. An educator, displayed as Teacher in the UI, can author cases and manage classes. An admin can administer the platform. Rank comparison allows permissions to evolve while equality checks on role strings stay out of the code, and it keeps a lower-ranked user from granting a role higher than their own.
...
A tenant is the organisational boundary inside a deployment. Users, sessions, courses, catalogue rows and logs are tenant-scoped. Tenant isolation is enforced through middleware, which covers every route from one place. Cross-tenant leakage would damage both privacy and research validity, so the boundary sits above individual handlers.

In a single-tenant deployment, this boundary is still active. In a multi-tenant deployment, it becomes the line between organisations. An admin is tenant-local. A teacher sees classes in their own tenant. Active sessions and logs are scoped. Catalogue rows may be user, tenant or platform scoped depending on governance. Moving a user between tenants leaves the content they authored where it was, which makes tenant reassignment a planned administrative operation.
...
Enrollment can happen in several ways. A teacher can add one student by username or email. A teacher can add students in bulk. A teacher can add co-teachers. Students can self-enrol with a join code. Re-adding a removed member revives the original membership, so the history stays in one row. Co-teacher promotion updates that same row, leaving the user's rank and record intact.

Join codes lower operational friction. A code allows students to attach themselves to the correct class after authentication. Codes can be copied, rotated or disabled. Anyone holding the code can join, which makes a code a convenience and leaves it short of a secret credential. If a code leaks, the teacher should rotate or disable it.
...
Assignment remains separate from scoring. Reports reflect the sessions students ran. Unassigning a case removes the live class link and leaves the case and its historical sessions in place. This separation allows teachers to change course structure while the record of what already happened stays intact.
...
For research, the same layer protects interpretation. It tells the researcher which learners belonged to which class, which cases were assigned, which attempts reached debrief and which data were exported. It makes it possible to compare learners under a shared assignment, with unrelated activity kept apart. In that sense, management is the structure that makes educational data meaningful.
