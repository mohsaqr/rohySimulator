---
layout: home
hero:
  name: Rohy
  text: Virtual Patient Simulation Platform
  tagline: Enterprise documentation. Pick the path that matches your role.
  actions:
    - theme: brand
      text: Product features
      link: /product/
    - theme: alt
      text: I am a trainee
      link: /trainee/
    - theme: alt
      text: I am an educator
      link: /educator/
    - theme: alt
      text: API reference
      link: /reference/api/
features:
  - title: Product features
    details: A scientific feature description of Rohy's modules, technologies, evidence streams and platform architecture.
    link: /product/
  - title: Trainee
    details: "Run a virtual-patient case: history, examination, investigations, treatment, vitals, voice mode and debrief."
    link: /trainee/
  - title: Educator
    details: Build a class, author cases with the wizard, design scenarios, and read the reporting & TNA analytics.
    link: /educator/
  - title: Administrator
    details: "Configure the platform safely: users & roles, settings, catalogue editors, multi-tenant operations."
    link: /admin/
  - title: Operator / DevOps
    details: Install, deploy, update and recover Rohy in production. Backups, migrations, retention, observability.
    link: /operator/
  - title: Integrator / Developer
    details: Call the API, embed the avatar kit, add providers, and extend the platform.
    link: /integrator/
  - title: Security & Compliance
    details: RBAC, audit chain, redaction & PII, retention, Oyon on-device emotion capture and EU AI Act posture.
    link: /security/
---

This documentation is organised by **who you are and what you are trying to
do**. Start from the card that matches your role.

## New here?

- **Trainees:** [Getting started](/trainee/getting-started): your first case in 10 minutes.
- **Educators:** [Classes & join codes](/educator/cohorts): set up a cohort and invite students.
- **Operators:** [Install](/operator/install) → [Deploy](/operator/deploy) → [Update](/operator/updating).
- **Integrators:** [API authentication](/integrator/api-auth) and the [API reference](/reference/api/).
