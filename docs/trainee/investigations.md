...
You order blood and other lab tests in the **Laboratory** room and imaging
...
1. In the catalogue (left), search or browse for what you need.
2. Tick each test or study you want. In the Laboratory you can also use the
   quick **panel** buttons (CBC, BMP, CMP, LFTs, Coags, Cardiac, Lipids,
   TFTs) to select a whole panel at once.
3. Click the order button to submit. A confirmation appears and the orders
   show up in the worklist.

Already-ordered items are greyed out and marked **Ordered** so you do not
order the same test twice.

::: tip Order from the patient room too
There is also a floating **Treatments / Records / Memory** strip on the
patient screen, and an order drawer behind it for labs and radiology. The
full-screen Laboratory and Radiology rooms are the primary place to order
and read results.
:::

## Turnaround

Results are not instant unless the case is set up that way. Each test has a
...
configured turnaround applies.

## View a result

1. In the worklist (right), click a row that is **Ready**.
...
Use the **Notes** button at the top of the room to record your
interpretation. Notes are saved with the session and available in the
