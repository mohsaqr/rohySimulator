...
No. Refreshing the page or closing and reopening the tab does **not** end
your session. The simulator restores your case, your room and your work when
you come back. A session ends only when you click **End & Debrief**, load a
different case, or your instructor ends it. If a case still looks empty after
a refresh, check you are signed in as the same user.
...
### "This session is open in another browser tab"

You have the same session open in more than one tab. The last tab to write
wins, which can overwrite chat history. Work in **one tab** at a time;
dismiss the banner once you have closed the extra tab.

### How do I switch to a different case?
...
### My lab or imaging results are not appearing
...
### I have no audio / the patient does not speak

Check, in order:

1. **Is voice mode on?** Click **Voice** at the top-right of the chat panel
   so it reads **Voice on**. If there is no Voice button, your instructor has
   not enabled voice and you should work in text.
...
The simulator shows the exact reason. Usual fixes: allow microphone access
for the site, serve the page over **HTTPS**, plug in a microphone, or speak
closer to it. See [Voice mode](/trainee/voice) for the full table.
...
### My old alarms are silent in a new case

Acknowledgements and snoozes belong to the case you set them in. Starting a
different case clears them, so new alarms in a new patient are not
accidentally silenced.

## Other characters

### A nurse / relative / consultant tab is disabled

Some characters must be summoned before they can talk. Use the **Call**
button on their tab; the message box stays disabled with a "not available"
note until they arrive. See [Taking a history](/trainee/history).
