...
This document covers **putting Rohy on a machine**: local dev, a single
classroom box, a multi-user systemd install, Docker, and air-gapped
...
|---|---|---|
| **Node.js** | 22.x (LTS) | Express 5 + Vite 7 require >=20; the project targets 22 |
| **npm** | 10.x | Bundled with Node 22 |
| **SQLite** | 3.x | System binary needed for the air-gap install path; the dev install uses bundled `sqlite3` npm package |
| **curl** | any | Required for the post-install Oyon model download (~93 MB from jsDelivr); skip with no internet, see the air-gap path |
...
**beside** the rohy checkout, as a sibling directory:
...
an empty stub, and the build breaks later, somewhere that looks unrelated.
That is why this item appears first in this document.

---

### The `3D` sibling: the Bedside package, also beside the checkout

`package.json` also declares `rohy-3d-patient-room` as `file:../3D`. It is a
plain ES-module package (no build step). Install the exact release pinned by
this Rohy checkout:

```bash
bash scripts/clone-room3d.sh ../3D                           # sibling, not nested
```

The helper reads `scripts/room3d-version.json`, uses `ROOM3D_GIT_TOKEN` as a
one-shot HTTP credential when the repository is private, and removes Git
metadata after cloning. `deploy/preflight.sh` checks for the package (step 9),
and `npm run build` refuses a missing or mismatched version.

---

## Published release (recommended)

If a release has been tagged, the fastest install pulls the pre-built
image. Building locally is slower. Every tag publishes three artifacts:
...
- **Default seeded users**: `admin` / `admin123`, `student` / `student123`.
  Production refuses them unless `ALLOW_DEFAULT_USERS=1`. Change them before any real user touches the box.

If `npm install` ran without `curl` or network, Oyon will not work until you re-fetch:
...
pixels: DICOM series and Deep Zoom tile pyramids. These are too large to live
in the repository or the image. They are published separately and installed
with one command:
...
**This step is required.** The default case orders four imaging
...
services hand out in place of a file are all refused before anything is
...
too, though [`bin/rohy-update`](/operator/updating) is the operator path for that.
...
For ad-hoc deploys, ephemeral test environments, or operators who would
...
| `--with-3d` | Bundle the `../3D` sibling clone (the Bedside package) if `package.json` uses `file:../3D` |
...
darwin-arm64 bundle will not accidentally get unpacked onto linux-x86_64.
...
To go beyond "is it up?" and verify that the Oyon validator runs
...
| `/etc/rohy/env` | `root` | systemd `EnvironmentFile`. Edit to change env vars such as `OYON_ENABLED` and `JWT_SECRET`. |
...
**nginx 502 immediately after install.** Express needs ~3-6s to finish migrations + seed checks. Wait, then `sudo systemctl status rohy`. If still 502 after 30s, check `journalctl -u rohy`, most often a missing env var is the cause.
