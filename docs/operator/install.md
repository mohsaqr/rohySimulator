...
services hand out instead of a file are all refused before anything is
...
affecting the others. Use a **fine-grained** token scoped to the content
repository alone with `Contents: Read`; a classic token's `repo` scope grants
read/write on every private repository the issuer owns.
...
If the content repository is ever made public, this section stops applying:
set `"private": false` in `scripts/content-sources.json` and
