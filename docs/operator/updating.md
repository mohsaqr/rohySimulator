...
This is the operator manual for keeping a rohy install current. If you are
looking only at the strategy or how it was designed, see the
[Update strategy](/operator/update-strategy). This document is the page you
read **before pressing the upgrade button**.

::: tip Reference
`bin/rohy-update` subcommands, exit codes and config vars are
single-sourced in [CLI & ops](/reference/cli/). Terms like *snapshot*,
*rollback recipe* and *additive migration* are defined in the
...
the strongest signal that the new version works.
...
also get a contract probe that POSTs an intentionally malformed emotion
...
Replace `<paste-password-here>` with the real password of the verifier
account. The credentials must belong to a real rohy user. Any role works:
the contract probe does not read data. It needs only to pass
...
Backups live at `/var/backups/rohy/<timestamp>-<git-sha>-<label>/`, where
`<timestamp>`, `<git-sha>` and `<label>` are filled in per snapshot.
...
In step 4, `<new-sha>` is the target commit `check` reported. Total wall
time: 30-120 seconds for a typical update, depending on `npm ci` cache
state and how much the build does.
...
broken, **do not wait**. Run `sudo rohy-update rollback` and report the issue.
...
run before the service restart: you are on the old version.
...
::: danger Destructive migrations block auto-rollback
...
code cannot read the new schema. Use `rohy-update restore-backup <snapshot>`
and accept that you may have to re-do anything that happened
since the apply.
:::
...
Replace the example name with one from `list-backups`. Restores DB.
Optionally restores the env file too. Restarts service.
...
**old code can always read the new schema** (see the
[Migrations runbook](/operator/migrations)).
...
install carries data you cannot afford to lose, set up an off-site copy.
...
### Option B: rclone to S3 / B2 / Google Drive / other backends
...
a backend with at-rest encryption and audit logs. See the
[Backup & restore runbook](/operator/backup-restore) for the full drill.
...
`rohy-update apply` should have already auto-rolled back. If it did not
(for example, the rollback itself failed), see "Manual recovery" below.
...
# 3. Restore DB (replace <snapshot> with the dir from step 2)
...
# Note the "from_sha" value, then (replace <from_sha>):
...
::: warning v1 does not verify release signatures
**v1 of `rohy-update` does NOT verify release signatures.** It pulls
from a github remote configured in your local clone. If the remote is
compromised, your install is compromised.
:::

Until signed releases ship (planned; see the
[Update strategy](/operator/update-strategy), Phase D), you can verify
what you are about to install manually:
...
If anything looks suspicious, such as a commit you do not recognize from the
maintainer, an unexplained binary blob, or a sudden `node_modules` change,
**do not apply it**. Open an issue first.

---

## What the tool does NOT do

- Does not auto-update on a schedule. You decide when.
- Does not send any telemetry. The tool does not phone home.
- Does not modify your `/etc/rohy/env` (except restoring during rollback).
- Does not touch user-uploaded data outside `/opt/data/rohy/database.sqlite`.
- Does not update Node, Python, system packages. Those are your distro's job.
- Does not manage TLS certs.
...
Do not include the full DB or `/etc/rohy/env` (it contains secrets).

---

## Further reading

- [Update strategy](/operator/update-strategy): design rationale, phases
  beyond v1, references.
- [Migrations runbook](/operator/migrations): migration policy
  + per-migration metadata.
- `README.md` in the repo: install paths.
- `HANDOFF.md` in the repo: current development state.
