...
Once you have installed (see [Install](/operator/install)), this document
...
| **Built from source** | Modifying rohy, or running on an architecture the project does not publish (ARM64 boards beyond the matrix) | `docker compose build` against the local checkout. Use this for `main` HEAD or for forks. |
...
- [ ] **HTTPS in front** of Express. Port 4000 stays off the public network: nginx, Caddy, or Cloudflare Tunnel sit in front of it.
...
- [ ] **Observability log shipper wired** to NDJSON stdout, or set `ROHY_LOG_LEVEL=warn` if you do not ship logs (see [Observability runbook](/operator/observability)).
...
| `ALLOW_DEFAULT_USERS` | unset | Set to `1` to keep seeded `admin/admin123` and similar accounts in production (do not use unless you are seeding and immediately rotating). |

### Observability

| Variable | Default | Effect |
|---|---|---|
| `ROHY_LOG_LEVEL` | `info` | `debug` / `info` / `warn` / `error`. Change to `warn` in production if you are not shipping logs. |
...
3. **Oyon API surface**: every Oyon route returns 401 (mounted) or 503 (disabled stub); any other status, including a bare 404, indicates a mounting problem
...
Without arming, `tech-test.sh` verifies only that routes are *mounted*. It
does not confirm that the validator catches malformed batches. To make every
deploy POST an intentionally malformed emotion batch and assert the validator
...
works: the contract probe does not read data. It needs only to pass
...
**Use cases**:
- After deploy: curl it. `total_5m` should be 0. A non-zero value means
  something in the new build is rejecting traffic.
...
on modern hardware: a Raspberry Pi 4 with WAL mode reaches ~100
sustained req/s, measured in `bench/`.
...
- **CSRF**: the SPA authenticates with bearer tokens. Classic CSRF does not apply to bearer-token auth. The `rohy_session` cookie exists only for the *standalone* Oyon page.
...
- **WAF / rate-limiter at the edge** (Cloudflare, AWS WAF, fail2ban with nginx logs). Express's rate-limiter is per-process; behind a load balancer it is per-instance.
- **Log shipper** for the NDJSON access log. SIEM, Loki, or `journalctl --output=cat` piped to S3 daily.
- **Backup encryption** if you are rsync'ing snapshots off-site (`gpg --symmetric` is fine).
- **Egress monitoring** if you have granted LLM API keys: set per-provider quotas in their dashboards.
...
- **No FIPS / HIPAA compliance attestation.** The codebase follows these requirements in principle but carries no formal certification.
...
| **Local TTS (Piper)** | Do not run `install-piper.sh`; **Settings → Voice** will hide Piper voices. |
| **Local TTS (Kokoro)** | Set **Voice → Provider** to anything else; Kokoro download skips. |
| **Cloud LLMs** | Do not set the provider's API key in Platform Settings. UI disables that provider. |
