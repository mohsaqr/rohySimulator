...
If anything fails the whole transaction rolls back, so a partial sweep
cannot happen.
...
The sweep is not automatic; it requires a cron job. Daily at 03:00:
...
The normal operator control is **Platform Settings → Retention** in
the admin UI; the env vars are an override for pinning the same
window in `/etc/rohy/env` regardless of DB state. A value of `0`
is honoured (everything past "now" is swept), so set it as an
explicit choice.
...
Oyon emotion records (`oyon_emotion_records`) use a separate window
from the global one above. Their retention is per tenant, read from
...
no `retention_days` configured keeps its Oyon records until one is set;
