...
::: tip Reference
The operator-facing CLI surface is single-sourced in
[CLI & ops](/reference/cli/); the migration policy lives in the
[Migrations runbook](/operator/migrations). Terms are locked in the
...
Lives in `migrations/MANIFEST.md`, mirrored as the
...
These terms are also in the canonical [Glossary](/reference/glossary):
