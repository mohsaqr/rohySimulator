...
- **Auto-run on boot.** The server applies any pending migration on
  start-up. A normal upgrade requires no manual migrate step: `npm
  run production` / `systemctl start rohy` does it.
- **Checksum-pinned.** If a file's contents change after it was applied,
  the runner aborts with `Migration checksum mismatch`, catching the
  drift immediately. A migration that has shipped stays as it is; a
  correction goes into a new migration.
- **Baseline stamping.** On a DB that already has the core tables but an
  ...
  of re-running it, so pre-existing installs are not re-bootstrapped.
...
Before any real apply, a dry-run confirms the pending set parses and
would apply cleanly without touching the DB:
...
this automatically as step 6, so a dry-run failure aborts the apply
before the real migration and the DB stays untouched.
...
`ROHY_DB=/path/to/database.sqlite` overrides the target DB if it is
...
- `CREATE TRIGGER` that leaves existing read/write semantics unchanged
- Default-value changes that leave existing rows untouched
...
| N+3 (>= 30 days later) | Drop the old. Mark the migration `destructive` here. |

At every step the previous release still runs without crashing.
Rollback is safe up to N+2; past N+3, restoring the destructive change
requires a pre-N+3 backup
...
4. `destructive`: refuses unless `--allow-destructive` is passed **and**
   the migration filename is typed to confirm.
5. `unknown` (migration not in the manifest): refuses to apply at
   all. It fails closed.

Adding a migration therefore also requires adding its manifest row in
the same commit. A release that ships a migration without a manifest
row makes `rohy-update` refuse the whole upgrade with:

```text
one or more migrations not declared in migrations/MANIFEST.md. Refusing to apply.
```

That is a bug in the release. Do not hand-edit the manifest to work
around it; wait for a fix release.
...
3. **Apply with the explicit gate:**
   ...
   Each destructive migration's filename must be typed to confirm.

::: danger Rollback is not automatic after a destructive apply
Once a destructive migration has run, old code cannot read the new
schema, so `rohy-update rollback` refuses. Recovery is a manual
