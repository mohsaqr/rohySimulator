...
Tenant scoping is enforced by middleware (`requireSameTenant`), which
replaces ad-hoc filters in each handler. In practice this means:
...
- **Active sessions** are visible and force-loggable only within your
  tenant.
- **Cohorts (classes)** belong to a tenant; a teacher sees the classes of
  their own tenant alone.
...
visible via the per-tenant **Base Class** backfill, a system cohort the
platform creates.
...
tenant as the deployment's identity: it is the only facility-name setting
(see [Platform settings](/admin/platform-settings)).
...
sessions). Full migration tooling is deferred to a later release. Reassigning a
...
There is no separate per-tenant settings screen. What is per-tenant today:
...
deployment-wide. If two organizations need different LLM keys or voice
providers, run separate deployments.
