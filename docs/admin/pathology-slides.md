...
tiles. Rohy keeps them outside the application. Slides live on a **content
origin**, and a case refers to one by name.
...
- **NEEDS CALIBRATION**: the file carried no scanner optics, and Rohy leaves
  them blank. Every measurement a reader makes is scaled by the magnification
  and the micrometres per pixel, so a plausible-looking default would make
  every measurement wrong by an unknown factor. The card offers **Add calibration**; type the two numbers once and
  the slide becomes `READY`.
- **FAILED**: the reason is shown on the card. **Remove** deletes the slide and
  everything derived from it.

An uncalibrated or failed slide is **withheld** when adding a slide to a
case. It is visible in the library so its state can be seen and acted on, and a
case can be built only around a slide whose scale is known.
...
| Largest slide that may be imported | 4 GiB | enforced on the declared size and again while downloading |
| Accepted slide formats | svs, ndpi, tiff, tif, dzi, zip | an *allow* set; a file's real type comes from reading it |
| Require scanner optics | **on** | off sends an uncalibrated slide straight to `READY` |
| Keep the original file | **on** | keeps the source so a slide can be re-tiled later |

**A tenant can only narrow the list.** The hosts you may choose from are set by
whoever runs the server (`ROHY_PLUGIN_IMPORT_ORIGINS`). Adding a host outside
that list is refused, naming the host. Choosing what the *server* will download
from is a decision about the machine's network position, which sits above one
tenant's teaching.
...
|---|---|---|
| Magnification to tile at | 10× | the tiler takes the strongest pyramid level at or below the target and stays within what the scanner captured |
...
front, because decoding a full-resolution plane would exhaust the server's
memory, and a conversion killed part-way leaves tiles that look complete.
...
2. **A library directory**, writable by the Rohy service user:
   ...
   Unset means no imports, and the settings page states that.
3. **An import allowlist**: `ROHY_PLUGIN_IMPORT_ORIGINS="pathology=https://slides.example.edu"`.
   A malformed value is fatal at boot, so a typo stops the server loudly and
   is visible at once.
...
by the same nginx block. It belongs to **Rohy**, and the content bundle leaves
it alone:
...
`GET /api/health/plugins` reports a `library` block per plugin. It carries
counts and bytes alone, with no names or source hosts, so a deploy check runs
without credentials:
...
which keeps "not configured" and "configured and empty" distinguishable.
...
- **Restarting Rohy mid-import** re-runs that import from the beginning at the
  next boot. A slide part-way through tiling looks like a finished one, so
  every interrupted import is redone.
